...
## Pre-flight — Clean checkout of base branch
...
| Condition | Action |
|-----------|--------|
| CI pending | **SKIP** — log `PR #<N>: CI still running, skipping`. Leave label. |
| CI failing + up-to-date | **SKIP** — log `PR #<N>: CI failing, skipping`. Leave label. |
| `CLEAN` or `UNSTABLE` | Go to **Step 3 — Merge** |
| `BEHIND` | Go to **Step 4 — Rebase** |
| `BLOCKED` (conflicts) | Go to **Step 4 — Rebase** |
...
- **Follow the repo's CLAUDE.md.** Read it before starting if you haven't already.
