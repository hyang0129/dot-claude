...
Detect whether `issue` is a number/URL or a free-form description:
- If it matches `^\d+$` or `^https?://github\.com/` → treat as GitHub issue reference.
...
**Always confirm before fetching:**
```
...
If `GIT_ROOT` is set, set up the scratch directory:
```bash
...
## Step 3 — Post to GitHub (if `--post`)

Only if `--post` was passed and the input was an issue reference.
