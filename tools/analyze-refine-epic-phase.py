#!/usr/bin/env python3
"""In-depth analysis of a single /refine-epic session's phase.

Usage:
    python analyze-refine-epic-phase.py <match> <phase> [--top N] [--md PATH]

Args:
    match   substring matching the session id or issue URL (e.g. "236" or "issues/236")
    phase   one of: scan | intv | post-chal | post-deco | post-surr
            (aliases for root-scan, root-interview, root-post-challenger, etc.)
...
# ── Load helpers from the sibling tool ───────────────────────────────────────
_HERE = os.path.dirname(os.path.abspath(__file__))
_spec = importlib.util.spec_from_file_location(
    "refine_epic_sessions", os.path.join(_HERE, "find-refine-epic-sessions.py"),
)
...
PHASE_ALIAS = {
    ...
    "post-chal":  "root-post-challenger",
    "post-deco":  "root-post-decomposer",
    "post-surr":  "root-post-surrogate",
    "pre":        "root-pre-cmd",
}
...
def walk_turns(filepath: str):
    """Yield one dict per assistant record, plus user-event markers for phase boundaries.

    Each dict has:
      kind         "assistant" | "user_cmd" | "user_reply" | "user_tool_result"
      ts           ISO timestamp string
      uuid         record uuid (for dedup)
      model        assistant model name (assistant only)
      usage        token usage dict (assistant only)
      tool_calls   list of {name, input} for tool_use blocks (assistant only)
      text         text content of assistant/user message (first ~500 chars)
    """
    # Dedup by message.id: one API response spans multiple JSONL records
    # (one per content block), each carrying identical usage. Merge all records
    # sharing a message.id into a single turn; fall back to uuid otherwise.
    events: list[dict] = []
        ...
        for line in f:
            ...
            if kind == "user" and msg.get("role") == "user":
                ...
                if not text.strip() and has_tool_result:
                    ...
                    continue
                if "<command-name>/refine-epic</command-name>" in text:
...
def compute_phase_boundaries(events: list) -> dict:
    """Return {phase_key: (start_ts_inclusive, end_ts_exclusive)} for root phases."""
    t_cmd = None
    for e in events:
        if e["kind"] == "user_cmd":
            t_cmd = e["ts"]
        ...
        if e["kind"] == "assistant":
            ...
            for tc in e["tool_calls"]:
                if tc["name"] in ("Agent", "Task"):
                    ...
                    if sa_phase == "challenger" and t_first_chal is None:
                        t_first_chal = e["ts"]
                    if sa_phase == "decomposer" and t_first_deco is None:
                        t_first_deco = e["ts"]
                    if sa_phase == "surrogate" and t_first_surr is None:
                        t_first_surr = e["ts"]
    ...
    boundaries = {
        ...
        "root-interview":       (t_first_user_reply or inf, t_first_chal or inf),
        "root-post-challenger": (t_first_chal or inf, t_first_deco or inf),
        "root-post-decomposer": (t_first_deco or inf, t_first_surr or inf),
        "root-post-surrogate":  (t_first_surr or inf, inf),
    }
    return boundaries
...
def main():
    ...
    p.add_argument("phase", help="scan|intv|post-chal|post-deco|post-surr|pre")
    p.add_argument("--days", type=int, default=30)
    p.add_argument("--top", type=int, default=5)
    p.add_argument("--md", default=None, help="write markdown to this path instead of stdout")
    args = p.parse_args()

    phase_key = PHASE_ALIAS.get(args.phase, args.phase)
    if not phase_key.startswith("root-"):
