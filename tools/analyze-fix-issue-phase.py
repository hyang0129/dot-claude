#!/usr/bin/env python3
"""In-depth analysis of a single /fix-issue session's phase or a subagent.

Usage:
    python analyze-fix-issue-phase.py <match> <phase> [--top N] [--md PATH]

Args:
    match   substring matching the session id or args (e.g. "42" or "abc1234")
    phase   one of:
              pre | setup | post-plan | post-arch | post-code |
              post-rev | post-doc | post-mer | done      (ROOT sub-phases)
              plan:<substr>  arch:<substr>  code:<substr>
              rev:<substr>   doc:<substr>   mer:<substr>
...
_spec = importlib.util.spec_from_file_location(
    "fi_sessions", os.path.join(_HERE, "find-fix-issue-sessions.py"),
)
...
PHASE_ALIAS = {
    "pre":        "root-pre-cmd",
    "setup":      "root-setup",
    "post-plan":  "root-post-planner",
    "post-arch":  "root-post-architect",
    "post-code":  "root-post-coder",
    "post-rev":   "root-post-reviewer",
    "post-doc":   "root-post-doc",
    "post-mer":   "root-post-mermaid",
    "done":       "root-skill-done",
}


def find_session(match: str, days: int):
    sessions = _fi.scan_sessions(days=days)
    low = match.lower()
    candidates = [
        s for s in sessions
...
def _transition_for(sa_phase: str) -> str | None:
    if sa_phase == "planner":
        return "root-post-planner"
    if sa_phase == "architect":
        return "root-post-architect"
    if sa_phase in _IMPL_PHASES:
        return "root-post-coder"
    if sa_phase == "reviewer":
        return "root-post-reviewer"
    if sa_phase == "doc":
        return "root-post-doc"
    if sa_phase == "mermaid":
        return "root-post-mermaid"
    return None
...
def analyze_root_phase(session: dict, phase_key: str, top_n: int) -> list[str]:
    events = list(walk_turns(session["session_file"], _fi.COMMAND_TAGS))
    ...
    for e in events:
        ...
        for tc in e["tool_calls"]:
            ...
            sa_phase = _fi.classify_subagent(desc)
            new_phase = _transition_for(sa_phase)
            if new_phase:
                transitions.append((e["ts"], new_phase))
                if new_phase == "root-post-doc" and t_first_doc is None:
                    t_first_doc = e["ts"]
    ...
    if t_first_doc is not None:
        for e in events:
            ...
            if e["kind"] == "user_reply":
                transitions.append((e["ts"], "root-skill-done"))
                break
...
def analyze_subagent(session: dict, sub_match: str, top_n: int) -> list[str]:
    ...
    if not matches:
        ...
        for sa in session["subagents"]:
            print(f"  {sa['phase']:>10}  {sa['agent_id'][:10]}  {sa['description']}", file=sys.stderr)
        sys.exit(1)
    if len(matches) > 1:
...
def _format_report(session, phase_key, assistant_events, top_n, subagent=None):
    for e in assistant_events:
        e["cost"] = _fi.cost_usd(e["usage"], e["model"])
    total_usage = _fi._empty_usage()
    for e in assistant_events:
    ...
    lines: list[str] = []
    lines.append(f"# /fix-issue phase analysis: {phase_key}\n")
    ...
    lines.append(
        ...
        f"hit%={_fi.cache_hit_pct(total_usage):.0f}%"
    )
    lines.append("")
    ...
    for rank, e in enumerate(top, 1):
        ...
        lines.append(
            ...
            f"({_fi._shorten_model(e['model'])}, t+{_hms(t0, e['ts'])})"
        )
        lines.append(
...
def main():
    ...
    p.add_argument("phase",
                   help="pre|setup|post-plan|post-arch|post-code|post-rev|post-doc|post-mer|done "
                        "| plan:|arch:|code:|test:|int:|chk:|rev:|doc:|mer:|sub:<substr>")
    ...
    ph = args.phase
    m = re.match(r"^([a-z]+):(.+)$", ph)
    if m and m.group(1) in _SUB_PREFIXES:
        sub_prefix = m.group(1)
        sub_match = m.group(2)
        # If a specific prefix was given, filter to subagents of that phase first.
        target_phase = _SUB_PREFIXES[sub_prefix]
        if target_phase is not None:
            low = sub_match.lower()
            scoped = [sa for sa in session["subagents"]
                      if sa["phase"] == target_phase
                      and (low in sa["description"].lower() or low in sa["agent_id"].lower())]
            if len(scoped) == 1:
                # Temporarily shadow subagents for analyze_subagent's matching.
                orig = session["subagents"]
                session["subagents"] = scoped
                try:
                    lines = analyze_subagent(session, sub_match, top_n=args.top)
                finally:
                    session["subagents"] = orig
            else:
                lines = analyze_subagent(session, sub_match, top_n=args.top)
        else:
            lines = analyze_subagent(session, sub_match, top_n=args.top)
